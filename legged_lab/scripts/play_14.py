...
#
# Play with Roban S14: 与 play.py 相同，但强制使用 assets/roban_s14（biped_s14.usd）进行播放。
...
parser = argparse.ArgumentParser(description="Play with Roban S14 (force S14 robot for roban_walk).")
parser.add_argument("--task", type=str, default=None, help="Name of the task (e.g. roban_walk).")
...
def play():
    ...
    # 强制使用 Roban S14 进行播放（与 walk_cfg 当前默认的 S2 解耦）
    env_cfg.scene.robot = ROBAN_S14_CFG
    print("[INFO] play_14: using Roban S14 (biped_s14.usd) for playback.")
